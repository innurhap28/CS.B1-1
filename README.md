...
---
## 03-3. 계정(agent-admin/dev/test) 및 그룹(agent-common/core) 생성
...
- 애플리케이션은 root가 아닌 `agent-admin` 계정으로 실행
- 실행 파일(bin)은 `agent-dev:agent-core` 소유로 관리
- 로그 및 API Key는 `agent-core` 그룹만 접근 가능
- 일반 테스트 계정 `agent-test`는 민감 데이터 접근 불가

이처럼 서비스 실행 계정과 파일 소유자를 분리하여 최소 권한 원칙을 적용하고, 
특정 계정이 침해되더라도 전체 시스템 권한으로 확장되는 것을 방지하고 역할별 접근 범위를 제한하였다. 
...
디렉토리 안에서 생성된 파일이 기존 권한 정책을 벗어나 **일관되지 않은 권한 구조**가 될 수 있다.
...
- agent-admin 계정을 통한 서비스 실행으로 보안성 확보
...
```
Health Check 과정에서 에러 발생 시 (비정상 상태) `exit 1`을 통해 종료
...
```
쉘 스크립트 기반 로그 로테이션 구현으로, 
로그 용량 관리 및 로그 파일 개수를 관리한다. 
